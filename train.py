...
def train_mask2former(model, train_loader, val_loader, criterion, optimizer, num_epochs=5, device="cuda"):
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Epoch [{epoch + 1}/{num_epochs}] Test - Loss: {avg_val_loss:.4f}, IoU: {avg_iou:.4f}, Dice: {avg_dice:.4f}, Pixel Acc: {avg_pixel_acc:.4f}\n")
...
def validate_mask2former(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for images, ground_truth_masks in tqdm(val_loader, desc="testing"):
...
def train_unet(model, train_loader, val_loader, criterion, optimizer, num_epochs=5, device="cuda"):
    ...
    for epoch in range(num_epochs):
        ...
        print(f"epoch [{epoch+1}/{num_epochs}] test - loss: {avg_val_loss:.4f}, iou: {avg_iou:.4f}, dice: {avg_dice:.4f}, pixel acc: {avg_pixel_acc:.4f}\n")
